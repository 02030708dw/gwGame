...
<!--    <slotFredHill v-if="contryId==1">
      ...
      <game-board :board-data="boardData1D as {
    label: string
    value: number
}[]" :active-data="activeData1D" :bg="boardBg" @onCheck="onAddAct1D"
      >
      </game-board>
    </slotFredHill>
    <slotFredHill v-if="contryId==2">
      ...
      <game-board :board-data="boardData2D as {
    label: string
    value: number
}[]" :active-data="activeData2D" :bg="boardBg" @onCheck="onAddAct2D"
      >
      </game-board>
    </slotFredHill>-->
    <gameFredHill @handleId="handleId1D2D3D" :backActive="oneActive" :TabData="TabDataTaiGuo" :urls="urls1"/>
    ...
    <view v-if="TabDataTwo1D2D3DID===0">
      <game-board :board-data="boardData1D as boardType[]" :active-data="activeData1D" :bg="boardBg" @onCheck="onAddAct1D"
      />
    </view>
    <view v-else-if="TabDataTwo1D2D3DID===1">
      <game-board :board-data="boardData2D as boardType[]" :active-data="activeData2D" :bg="boardBg" @onCheck="onAddAct2D"
      />
    </view>
    <view v-else-if="TabDataTwo1D2D3DID===2">
      <game-board :board-data="boardData3D as boardType[]"
                  :active-data="activeData3D"
                  :active-sub-data="activeSubData3D"
                  :board-sub-data="boardSubData3D"
                  @on-sub-check="onAddActSub3D"
                  :bg="boardBg" @onCheck="onAddAct3D"
      />
    </view>
    <template #bot>
      <gameFooter/>
...
import boardBg from '@/static/images/fredHill3M.png'
...
const {
  contryId, TabData, TabDataTaiGuo, oneActive, twoActive,
  threeActive, TabDataTwo, TabDataAll, TabDataTwo1D2D3D,TabDataTwo1D2D3DID

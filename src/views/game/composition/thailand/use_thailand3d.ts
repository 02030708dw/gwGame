...
export default function (type: string, lotteryHistory: Map<string, Ref<{}>>, playTypeCode: Ref, lock: Ref) {
    ...
    const boardSubData3D = ref(calData(10).map(it =>
        ({label: it.label + '00', value: it.value, temp: [] as number[]})).map(it => ({
        ...it,
        range: Number(it.label) + 99
    })))
    ...
    const onAddActSub3D = (i: number) => {
        ...
        if (activeSubData3D.value.includes(i)) activeSubData3D.value.splice(activeSubData3D.value.findIndex(it => it === i), 1)

	...
	<u-popup :show="show" :round="10" mode="bottom" @close="close">
		<view class="bet-box">
			<view class="top">
				...
				<image class="close" src="/src/static/images/close.png" @click="close" />
			</view>
			<scroll-view scroll-y="true" class="scroll-view">
				<view class="item" v-for="(item,index) in list" v-if="list.length" :key="item.id">

					<text class="title">[{{ item.gamePlayName}},{{item.gamePlayTypeName}}]</text>

					<text class="num">
						{{ item.betNums.slice(0,2).join(',') }}
						<text v-if="item.betNums.length>2" class="more" @click="more(item,index,$event)">更多</text>
					</text>
					...
					<view class="dialog-box" v-if="showDetail==item.id" tabindex="-1" @blur="showDetail=false"
						:style="{top:top+'rpx'}">
						<view class="title">共{{ item.betNums.length }}注
			...
			<view class="bottom">
				...
				<image src="@/static/images/footerq1.png" style="width: 64rpx; height: 64rpx; margin-left: 100rpx" />
				<image src="@/static/images/footerq2.png" style="width: 64rpx; height: 64rpx; margin-left: 16rpx" />
